// Social share — Web Share API (native share sheet) on mobile, web URLs on desktop
...
export function shareOnPlatform(platform: SocialPlatform, text: string, url: string): void {
  const encodedText = encodeURIComponent(text);
  const encodedUrl = encodeURIComponent(url);
  const encodedFull = encodeURIComponent(text + ' ' + url);

  // On mobile: Web Share API → OS-level share sheet → user picks the app
  // This is the ONLY reliable cross-platform way to share content into specific apps
  if (canUseWebShare()) {
  ...
  // On desktop: open platform-specific web share URLs
