...
import { useOrders, useSettings, bindReferral, generateLoyaltyCoupon, useMyCoupons, useReferredUsers } from '@/hooks/useFirestoreData';
...
import { User, MapPin, Package, Gift, Settings, Star, Copy, Check, ExternalLink, Clock, Shield, FileText, RotateCcw, Save, Ticket, Link2 } from 'lucide-react';
...
export default function ProfilePage() {
  ...
  const { referred } = useReferredUsers(userData?.referralCode);
  ...
  const handleBindReferral = async () => {
    ...
    try {
      ...
      setReferralMsg('রেফারেল কোড যুক্ত হয়েছে! আপনার রেফারার পয়েন্ট পাবেন আপনি প্রথম অর্ডার সম্পন্ন করলে।');
      setReferralInput('');
      ...
      <Tabs defaultValue={defaultTab}>
                ...
                <div className="flex items-center justify-between mt-1">
                  ...
                  <div className="flex items-center gap-2">
                    {order.status === 'delivered' && (
                      <Link to="/support" onClick={e => e.stopPropagation()} className="text-xs text-primary font-medium flex items-center gap-1">
                        <RotateCcw size={12} /> Return
                      </Link>
                    )}
                    <span className="text-xs text-primary flex items-center gap-1"><ExternalLink size={12} /> Details</span>
                  </div>
                </div>
              </Link>
            ))}
        ...
        <TabsContent value="rewards" className="space-y-4">
          ...
          <div className="bg-card border border-border rounded-xl p-4">
            ...
            <p className="text-xs text-muted-foreground mb-3">আপনার রেফারেল লিংক শেয়ার করুন। যাকে রেফার করবেন তিনি প্রথম অর্ডার সম্পন্ন করলে আপনি {settings.referralPoints ?? 50} পয়েন্ট পাবেন।</p>
                ...
                <div className="text-xs text-muted-foreground flex flex-wrap items-center gap-x-1.5 gap-y-1">
                  ...
                  {referred.map((r, i) => (
                    <span key={r.id} className="inline-flex items-center gap-0.5">
                      <span className="font-mono">{r.referralCode}{i < referred.length - 1 ? ',' : ''}</span>
                      {r.rewarded ? (
