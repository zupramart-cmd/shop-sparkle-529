...
export default function Header() {
  ...
  // Handle icon click: install PWA if installable, otherwise navigate to home
    ...
    <>
      <header className="sticky top-0 z-40 bg-card/95 backdrop-blur border-b border-border">
        ...
        <div className="lg:hidden flex items-center gap-3 px-4 h-14">
          ...
          <Link to="/" className="flex-1 flex items-center" onClick={handleIconClick}>
            <img
              src="/icon.png"
              alt={settings.appName}
              className={['h-12 max-w-[120px] object-contain transition-opacity duration-150', isInstallable ? 'active:opacity-70' : ''].join(' ')}
              onError={e => { (e.target as HTMLImageElement).src = '/logo.jpg'; }}
              title={isInstallable ? 'অ্যাপ ইন্সটল করুন' : settings.appName}
            />
          </Link>
        ...
        <div className="hidden lg:flex items-center gap-6 px-6 h-16 max-w-screen-xl mx-auto">
          <Link to="/" className="flex items-center shrink-0" onClick={handleIconClick}>
            <img
              src="/icon.png"
              alt={settings.appName}
              className={['h-12 max-w-[160px] object-contain transition-opacity duration-150', isInstallable ? 'active:opacity-70 cursor-pointer' : ''].join(' ')}
              onError={e => { (e.target as HTMLImageElement).src = '/logo.jpg'; }}
              title={isInstallable ? 'অ্যাপ ইন্সটল করুন' : settings.appName}
            />
          </Link>
